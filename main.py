...
def main(plot: bool = False) -> None:
    ...
    if "model" in config:
        ...
        eval_cfg = config.get("evaluations", {}).get(
            "overview_last_fold"
        ) or config.get("evaluations", {}).get("tufte_last_fold", {})
        horizon = int(eval_cfg.get("horizon", 12))
        n_splits = int(eval_cfg.get("n_splits", 5))
        models_config = config.get("models", {}).get(
            "overview_last_fold"
        ) or config.get("models", {}).get("tufte_last_fold", [])
    ...
    if best_result.y_true is not None and best_result.y_pred is not None:
        if plot:
            ...
            ax.plot(
                ...
                best_result.y_true.values().flatten(),
                "b-",
                lw=1.8,
                label="Actual",
                alpha=0.8,
            ...
            ax.plot(
                best_result.y_pred.time_index,
                best_result.y_pred.values().flatten(),
                "r--",
                lw=2.0,
